...
class face_localizer:
    def __init__(self):
        ...
        self.face_net = cv2.dnn.readNetFromCaffe(rospy.get_param('~proto_loc', '') + 'deploy.prototxt.txt', rospy.get_param('~proto_loc', '') + 'res10_300x300_ssd_iter_140000.caffemodel')
        ...
        for i in range(0, face_detections.shape[2]):
            ...
            if confidence>MIN_CONF:
                ...
                if pose is not None:
                    ...
                    acc_x = math.floor((pose.position.x + MAP_ORIGIN[0]) / ACC_RESOLUTION)
                    acc_y = math.floor((pose.position.y + MAP_ORIGIN[1]) / ACC_RESOLUTION)
                    print("x={:f}, y={:f}, acc_x={:f}, acc_y={:f}".format(pose.position.x, pose.position.y, acc_x, acc_y))
                    print(self.prev_detect)
