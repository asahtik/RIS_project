...
void find_rings(const sensor_msgs::ImageConstPtr &rgb_img, const sensor_msgs::PointCloud2ConstPtr &depth_blob) {
  ...
  for(std::list<std::tuple<cv::RotatedRect*, cv::RotatedRect*>>::iterator it = candidates.begin(); it != candidates.end(); ++it) {
    ...
    float x1 = (box[3].x < box[2].x) ? box[3].x : box[2].x, x2 = (box[1].x > box[0].x) ? box[1].x : box[0].x;
    if(x1 <= 0) x1 = 0;
    if(x2 > width) x2 = width;

    // float y1 = center.y + b + 1, y2 = center.y - b;
    float y1 = (box[1].y > box[2].y) ? box[1].y : box[2].y, y2 = (box[0].y < box[3].y) ? box[0].y : box[3].y;
    if(y2 <= 0) y2 = 0;
    if(y1 > height) y1 = height;

    // ROS_WARN("0 %f %f, 1 %f %f, 2 %f %f, 3 %f %f", box[0].x, box[0].y, box[1].x, box[1].y, box[2].x, box[2].y, box[3].x, box[3].y);
    // ROS_WARN("x1 %f, x2 %f, y1 %f, y2 %f", x1, x2, y1, y2);
  ...
  if(no_markers > 0) {
    ...
    clustering2d::to_cluster_array_msg(ring_c, carr);
